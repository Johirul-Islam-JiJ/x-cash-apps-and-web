...
@section('content')
    <div class="col-xl-10">
        <div class="custom--card">
            <div class="card-header d-flex flex-wrap align-items-center justify-content-center bg--base text-white">
                <div class="bank-icon  me-2">
                    <i class="las la-hand-holding-usd"></i>
                </div>
                <h4 class="fw-normal text-white">@lang('Money Out')</h4>
            </div>
                            ...
                            <div class="p-4 border mb-4">
                                <div class="row">
                                    <div class="col-lg-12 form-group">
                                        <label class="mb-0">@lang('Agent Username/E-mail')<span class="text--danger">*</span>
                                        </label>
                                        <div class="input-group">
                                            <input type="text" class="form--control checkUser" id="username"
                                                name="agent" placeholder="@lang('Agent Username/E-mail')" value="{{ old('agent') }}"
                                                required>
                                            <button type="button" class="input-group-text scan" data-toggle="tooltip"
                                                title="@lang('Scan QR')"><i class="las la-camera"></i></button>
                                        </div>
                                        ...
                                        <select class="select style--two currency" name="wallet_id" required>
                                            ...
                                            @foreach ($wallets as $wallet)
                                                <option value="{{ $wallet->id }}"
                                                    ...
                                                    {{ $wallet->currency->currency_code }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-lg-12 form-group">
                                        ...
                                        <input type="number" step="any" class="form--control style--two amount"
                                            name="amount" placeholder="@lang('Amount')" required
                                            value="{{ old('amount') }}">
                                    </div>
                                    <label>
                                        <span
                                            class="text--warning min">@lang('Min: '){{ getAmount(@$moneyOutCharge->min_limit) }}
                                            {{ defaultCurrency() }} --</span>
                                        <span
                                            class="text--warning max">@lang('Max: '){{ getAmount(@$moneyOutCharge->max_limit) }}
                                            {{ defaultCurrency() }}</span>
                                    </label>
                                </div><!-- row end -->
                            </div>
                    ...
                    <div class="d-widget border-start-0 shadow-sm">
                        <div class="d-widget__content">
                            <ul class="cmn-list-two text-center mt-4">
                                <li class="list-group-item">@lang('Total Amount'): <strong class="total_amount"></strong>
                                </li>
                                <li class="list-group-item">@lang('Total Charge'): <strong class="total_charge"></strong>
                                </li>
